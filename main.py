...
player_imgs = [pygame.transform.scale(pygame.image.load(IMGS_PATH + '/' + file).convert_alpha(), (100, 50)) for file in listdir(IMGS_PATH)]
...
def create_enemy():
    ...
    enemy_rect = pygame.Rect(wight, random.randint(0, height), *enemy.get_size())
    enemy_speed = random.randint(2, 5)
    return [enemy, enemy_rect, enemy_speed]

def create_bonus():
    bonus = pygame.transform.scale(pygame.image.load('bonus.png').convert_alpha(), (50,80))
    bonus_rect = pygame.Rect(random.randint(100,wight - 100), 0, *bonus.get_size())
...
while is_working:
    ...
    main_surface.blit(font.render(str(scores),True , BLACK), (wight - 30, 0))
    ...
    if pressed_key[K_RIGHT] and not player_rect.right >= wight:
